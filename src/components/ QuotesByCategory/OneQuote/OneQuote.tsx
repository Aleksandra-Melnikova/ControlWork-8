...
const OneQuote: React.FC<OneQuoteProps> = ({
  quoteText,
  author,
  onEdit,
  onDelete,
  ...
  return (
    <div className="border border-1 border-secondary rounded-2 p-4 row mb-3 text-start justify-content-between align-items-start fs-4">
      ...
      <button className="btn btn-outline-secondary col-1 ms-5" onClick={onEdit}>
        Edit
      </button>
      <button className="btn btn-close col-1 mt-2" onClick={onDelete}></button>
    </div>
  );
};
